...
[<Fact>]
let ``toString of singleton tree`` () =
    Property.check <| property {
        let! v = Gen.uint64 (Range.exponentialBounded ())
        let tree = Tree.singleton v
        let expected = sprintf "%A" v
        test <@ expected = Tree.toString tree @>
    }

[<Fact>]
let ``toStringList of binary tree with depth 1`` () =
    Property.check <| property {
        let! v = Gen.uint64 (Range.exponentialBounded ())
        let! v0 = Gen.uint64 (Range.exponentialBounded ())
        let! v1 = Gen.uint64 (Range.exponentialBounded ())
        let tree = Node (v, [ v0; v1 ] |> Seq.map Tree.singleton)
        let expected = [
            sprintf "%A" v
            sprintf " ├-%A" v0
            sprintf " └-%A" v1
        ]
        test <@ expected = Tree.toStringList tree @>
    }

[<Fact>]
let ``toStringList of binary tree with depth 2`` () =
    Property.check <| property {
        let! v = Gen.uint64 (Range.exponentialBounded ())
        let! v0 = Gen.uint64 (Range.exponentialBounded ())
        let! v1 = Gen.uint64 (Range.exponentialBounded ())
        ...
        let tree = Node (v, [ v0Children; v1Children ])
        let expected = [
            sprintf "%A" v
            sprintf " ├-%A" v0
            sprintf " |  ├-%A" v00
