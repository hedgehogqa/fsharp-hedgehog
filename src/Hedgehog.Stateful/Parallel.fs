...
module Parallel =
    ...
    let internal genActions
        ...
        gen {
            // Generate setup actions first and get the projected state and environment
            let! setup, stateAfterSetup, envAfterSetup =
                Sequential.genActionsWithState (Range.singleton 0) setupActions [] [] initialState Env.empty

            // Generate prefix using state and environment after setup and get the projected state and environment
    ...
    let internal isLinearizable
        ...
        let rec searchInterleavings state env xs ys =
            ...
            | x :: xs', y :: ys' ->
                ...
                match tryExecuteAction state env x with
                | Some(state', env') when searchInterleavings state' env' xs' ys -> true // Found valid path through branch1 first
                | _ ->
    ...
    let internal executeWithSUT (sut: 'TSystem) (actions: ParallelActions<'TSystem, 'TState>) : Property<unit> =
        ...
        property {
            ...
            let runBranch (branch: Action<'TSystem, 'TState> list) : Async<Result<(Name * obj) list, exn>> =
                let rec loop results branchEnv branchState =
                    function
                    | [] -> async { return Ok(List.rev results) }
                    | action :: rest ->
                        async {
                            // Skip actions that don't satisfy preconditions
                            if not (action.Precondition branchState && action.Require branchEnv branchState) then
                                return! loop results branchEnv branchState rest
                            else
                                let! result = action.Execute sut branchEnv branchState |> Async.AwaitTask

                                match result with
                                | ActionResult.Failure ex -> return Error ex
                                | ActionResult.Success output ->
                                    let outputVar = Var.bound action.Id
            ...
            let! branchResults =
                async { return! Async.Parallel [ runBranch actions.Branch1; runBranch actions.Branch2 ] }
            ...
            let linearizabilityCheck =
                ...
                | Ok results1, Ok results2 ->
                    let allResults =
                        [ yield! prefixResults; yield! results1; yield! results2 ] |> Map.ofList
