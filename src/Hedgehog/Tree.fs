...
module Tree =
    ...
    let rec toStringList (Node (x, xs) : Tree<'a>) : string list =
    ...
    let toString (t : Tree<'a>) : string =
        toStringList t |> Seq.reduce (fun a b -> a + System.Environment.NewLine + b)